...
function ReviewPage({ snapshot, updateSnapshot }: PageProps) {
  const docs = snapshot.activeBatch?.documents.filter((item) => item.phase === 'EN REVISIÓN') ?? [];
  ...
  const save = async () => {
    ...
    if (result.ok && result.data && snapshot.activeBatch) {
      updateSnapshot({ activeBatch: { ...snapshot.activeBatch, documents: snapshot.activeBatch.documents.map((item) => item.id === result.data!.id ? result.data! : item) } });
      setSelected(result.data);
    }
  };
  return <>
    <SectionHeader eyebrow="Control humano" title="Revisión manual" description="Los casos dudosos permanecen aquí hasta que exista evidencia suficiente o una decisión explícita."/>
    {!docs.length ? <EmptyState icon="check" title="No hay revisiones en el lote activo">Las revisiones históricas siguen disponibles desde Facturas.</EmptyState> : <section className="review-layout">
      ...
      {selected && <div className="review-detail"><div className="review-detail__head"><div><p className="eyebrow">Documento · {selected.id}</p><h2>{selected.originalName}</h2><p>{selected.subject}</p></div><a className="button button--secondary" href={selected.gmailUrl} target="_blank" rel="noreferrer"><Icon name="mail" size={17}/>Abrir correo</a></div><EvidenceChain active={3}/>
        ...
        <div className="review-actions"><Button variant="secondary" onClick={() => setSelected({ ...selected, proposedStatus: 'NO ES FACTURA' })}>No es factura</Button><Button variant="secondary" onClick={() => setSelected({ ...selected, proposedStatus: 'FACTURA DE VENTA' })}>Factura de venta</Button><Button icon="refresh" onClick={save}>Guardar y reevaluar</Button></div>
      </div>}
    </section>}
  </>;
}
...
function SuppliersPage({ snapshot, updateSnapshot }: PageProps) {
  ...
  const [editing, setEditing] = useState<Supplier | null>(null); const [showInactive, setShowInactive] = useState(false); const [query, setQuery] = useState('');
  ...
  return <>
    <SectionHeader eyebrow="Catálogo acreditado" title="Proveedores" description="Solo los proveedores activos y respaldados por evidencia pueden identificar facturas automáticamente." action={<><Button variant="secondary" icon="download" onClick={exportCsv}>Exportar</Button><Button icon="plus" onClick={() => setEditing(empty)}>Añadir proveedor</Button></>}/>
...
function SettingsPage({ snapshot, updateSnapshot }: PageProps) {
  const [settings, setSettings] = useState(snapshot.settings); const [saved, setSaved] = useState(false); const [setupResult, setSetupResult] = useState(''); const [settingUp, setSettingUp] = useState(false);
  const save = async () => { const result = await api.updateSettings(settings); if (result.ok && result.data) { updateSnapshot({ settings: result.data }); setSaved(true); setTimeout(() => setSaved(false), 2500); } };
