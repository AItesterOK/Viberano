...
function buildMonthlyClose_(period) {
  ...
  const grouped = {};
  invoices.forEach(function (invoice) { const id = invoice.categoryId || ''; if (!grouped[id]) grouped[id] = { categoryId: id, category: categoryMap[id] || 'Sin categoría', count: 0, totalCents: 0 }; grouped[id].count += 1; grouped[id].totalCents += toCents_(invoice.total); });
  const taxSummary = invoices.reduce(function (sum, invoice) { sum.baseCents += toCents_(invoice.taxableBase); (invoice.taxLines || []).forEach(function (line) { if (String(line.kind) === 'RETENCION') sum.withholdingsCents += Math.abs(toCents_(line.amount)); else sum.taxesCents += toCents_(line.amount); }); sum.totalCents += toCents_(invoice.total); return sum; }, { baseCents: 0, taxesCents: 0, withholdingsCents: 0, totalCents: 0 });
  ...
  return { period: month, coverage: coverages.length ? coverages[0] : 'Sin extracto confirmado', invoices: invoices.length, reviews: documents.length, reconciled: reconciled, partial: partial, excluded: excluded, movementsWithoutInvoice: withoutInvoice, invoicesWithoutMovement: invoices.filter(function (invoice) { return !invoice.reconciliationStatus || invoice.reconciliationStatus === 'SIN CONCILIAR'; }).length, taxableBase: taxSummary.baseCents / 100, taxes: taxSummary.taxesCents / 100, withholdings: taxSummary.withholdingsCents / 100, total: taxSummary.totalCents / 100, warnings: warnings, byCategory: Object.keys(grouped).map(function (id) { const item = grouped[id]; return { categoryId: item.categoryId, category: item.category, count: item.count, total: item.totalCents / 100 }; }) };
}
